...
| **Validações executadas** | Baseline pré-mudança: `npm run lint` — **PASS** (warnings pré-existentes), `npm run build` — **PASS**, `npm test -- --runInBand` — **PASS**. Pós-mudança: `npm run lint`, `npm run build`, `npm test -- --runInBand` (registrados no PR). |
