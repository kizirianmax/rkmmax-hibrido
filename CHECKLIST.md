...
### O que mudou
- Criada arquitetura limpa para integração GitHub em `api/lib/github/`:
  - `githubConfig.js` — feature flag (`GITHUB_INTEGRATION_ENABLED`) + leitura segura de env vars
  - `githubClient.js` — HTTP wrapper sobre `fetch` nativo (timeout 8 s, retry em 5xx, sem leak de tokens em logs)
  - `githubService.js` — funções: `listRepos`, `listBranches`, `getFile` (reais); `putFile`, `createPR` (stubs NotImplemented)
  - `githubTypes.js` — JSDoc/types para padronizar retornos
- Criado `api/github.js` — endpoints mínimos e seguros:
  - `GET /api/github/status` → `{ enabled, mode, message }`
  - `GET /api/github/repos` → lista de repos ou erro claro (501/400/401)
- Adicionados 18 testes unitários em `api/__tests__/github.test.js`
- Atualizado `.env.example` com `GITHUB_INTEGRATION_ENABLED=false` e `GITHUB_AUTH_MODE=stub`

### Por quê
- Prepara o /hybrid para evoluir para Construtor (N2/N3): lê/cria arquivos, roda build, abre PR
- Feature flag garante que nada de GitHub real executa em produção sem configuração explícita
- Arquitetura isolada (adapter/service) facilita substituição de auth sem tocar nos endpoints

### Arquivos alterados

| Arquivo | Mudança |
|---|---|
| `api/lib/github/githubConfig.js` | NOVO — feature flag + credenciais |
| `api/lib/github/githubClient.js` | NOVO — HTTP wrapper com retry/timeout |
| `api/lib/github/githubService.js` | NOVO — listRepos/listBranches/getFile + stubs putFile/createPR |
| `api/lib/github/githubTypes.js` | NOVO — JSDoc types |
| `api/github.js` | NOVO — endpoints GET /status e GET /repos |
| `api/__tests__/github.test.js` | NOVO — 18 testes unitários |
| `.env.example` | Adicionadas variáveis GITHUB_INTEGRATION_ENABLED e GITHUB_AUTH_MODE |
...
|---|---|---|
| `GITHUB_INTEGRATION_ENABLED` | `false` | Feature flag — habilitar apenas com credenciais configuradas |
| `GITHUB_AUTH_MODE` | `stub` | Modo de auth: `stub` (mock), `oauth` (token pessoal), `app` (GitHub App — TODO) |
| `GITHUB_APP_ID` | _(vazio)_ | ID do GitHub App (futuro) |
| `GITHUB_APP_PRIVATE_KEY` | _(vazio)_ | Chave privada do GitHub App (futuro) |
| `GITHUB_APP_INSTALLATION_ID` | _(vazio)_ | Installation ID (futuro) |

### Como testar localmente

```bash
# 1. Flag desabilitada (padrão)
curl http://localhost:3000/api/github/status
# → { "enabled": false, "mode": "stub", "message": "GitHub integration is disabled…" }

curl http://localhost:3000/api/github/repos
# → 501 { "error": { "code": "INTEGRATION_DISABLED", "message": "…" } }

# 2. Flag habilitada, modo stub (sem credenciais reais)
GITHUB_INTEGRATION_ENABLED=true GITHUB_AUTH_MODE=stub node -e "…"
curl http://localhost:3000/api/github/repos
# → 200 [ { "id": 1, "fullName": "kizirianmax/rkmmax-hibrido", … } ]

# 3. Flag habilitada, oauth, sem token
GITHUB_INTEGRATION_ENABLED=true GITHUB_AUTH_MODE=oauth curl http://localhost:3000/api/github/repos
# → 400 { "error": { "code": "CREDENTIALS_MISSING", "message": "GITHUB_TOKEN is not set" } }
```

### TODOs para Construtor N2/N3

1. `githubConfig.js` — implementar leitura de `GITHUB_APP_ID` + `GITHUB_APP_PRIVATE_KEY`
2. Criar `GET /api/github/app/install` — fluxo de instalação do GitHub App
3. Criar `GET /api/github/app/callback` — recebe `installation_id`, salva no Supabase
4. `githubClient.js` — adicionar geração de JWT (App) → troca por installation token
5. `githubService.js` — implementar `putFile` (PUT `/repos/{owner}/{repo}/contents/{path}`)
6. `githubService.js` — implementar `createPR` (POST `/repos/{owner}/{repo}/pulls`)

### Rollback
```bash
git revert <sha-deste-commit>
# Remove api/github.js, api/lib/github/, api/__tests__/github.test.js
# Reverte .env.example e CHECKLIST.md
```
