...
| **Validações executadas** | Baseline pré-mudança: `npm run lint` — **PASS** (warnings pré-existentes), `npm run build` — **PASS**, `npm test -- --runInBand` — **PASS**. Pós-mudança: `npm run lint` — **PASS** (warnings pré-existentes), `npm run build` — **PASS**, `npm test -- --runInBand` — **PASS**. Testes direcionados: `npm test -- --runInBand src/__tests__/public-pages-premium-ui.test.js src/__tests__/demo-showcase-routing.test.js` — **PASS**. Verificação manual local com Playwright em desktop e mobile: `/startup` permanece pública (sem redirecionamento para login) e CTA principal abre `/demo` pública. |
