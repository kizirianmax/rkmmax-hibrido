...
| **Objetivo** | Conectar o diagnóstico verdict-only ao fluxo real de preview (`POST /api/artifact-preview`) usando somente `preview.summary` (`fileContents`, `id`, `version`) e `entrypoint` inferido, sem UI e sem execução. |
| **Ponto de wiring em runtime** | `src/pages/HybridAgentSimple.jsx`: no sucesso de `handleGeneratePreview` (`data.success && data.preview`), chama `observeConstructorRealPreviewDiagnostic(data.preview)` exatamente uma vez e armazena o resultado em `realPreviewDiagnosticsRef` (somente observacional local, sem renderização). |
| **Retorno do diagnóstico** | Permanece verdict-only via adaptador #594 (`eligible/unavailable`), sem expor payload bruto. |
| **Confirmações** | Sem UI nova; sem execução real; sem API nova; sem storage novo; sem uso de `agentMessage.content`, `contentPreview` ou `zipBase64` para o diagnóstico; contratos #581–#594 intactos; `executeArtifact` server-side permanece `disabled`; sem bypass ao Serginho; Dependabot fora de escopo. |
