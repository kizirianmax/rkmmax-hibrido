...
| **Comportamento** | Novo classificador puro recebe mapa seguro `path -> content` (com `entrypoint`/`validation` opcionais), ignora metadados internos (`README.md`, `manifest.json`, `logs/*`) como sinal primário, aplica regras conservadoras de bloqueio (path perigoso, rede/import externo, `eval`/`Function`) e retorna apenas veredito serializável (`capability`, `reasons`, `flags`) sem payload bruto. |
