...
- **Serginho GitHub Execution Checklist (N11)**: criado `api/lib/serginho/analysis/githubExecutionChecklist.js` com `isExecutionChecklistFollowUp(message)` (detecta perguntas de checklist/tarefas/lista executável em PT-BR e EN por regex: "me dê um checklist", "lista de tarefas", "quebra isso em tarefas", "give me an actionable checklist", "break this into tasks", etc.), `hasEnoughContextForChecklist(githubContext)` (verifica se há ao menos um campo de contexto disponível — mesmo critério do N10), `buildChecklistPrompt(message, githubContext)` (monta prompt estruturado com contexto atual + contexto anterior se disponível + instruções de checklist com prioridade/dependência/critério de conclusão + anti-alucinação), `formatChecklistResponse(rawText, options)` (pós-processa resposta LLM: adiciona cabeçalho `## Checklist sugerido`, rodapé com fonte, truncamento seguro com `[resposta truncada]`, redação de tokens `sk-*`/`ghp_*`/`Bearer`), `getInsufficientChecklistContextMessage()` (mensagem amigável orientando o usuário); orchestrator minimamente modificado com bloco checklist follow-up ANTES do bloco N10 (action plan), guarda `_skipExecutionChecklistCheck` anti-loop, `_meta.executionChecklistFollowUp = true` e `_meta.executionChecklistFormatted = true`; testes cobrindo todos os cenários. Zero dependências novas. Zero breaking changes. Rollback com `git revert`.
- **Serginho GitHub Action Plan (N10)**: criado `api/lib/serginho/analysis/githubActionPlan.js` com `isActionPlanFollowUp(message)` (detecta perguntas de plano/roadmap/sequência em PT-BR e EN por regex), `hasEnoughContextForActionPlan(githubContext)`, `buildActionPlanPrompt(message, githubContext)` (prompt com instruções de prioridade/impacto/esforço/risco e indicador de contexto parcial), `formatActionPlanResponse(rawText, options)` (pós-processa com header, rodapé, truncamento, sanitização de tokens), `getInsufficientActionPlanContextMessage()`; orchestrator integrado com guarda `_skipActionPlanCheck` antes do bloco N9; 100 novos testes em `api/__tests__/serginho-github-action-plan.test.js`. Zero dependências novas. Zero breaking changes. Rollback com `git revert`.
- **Serginho GitHub Action Recommendations (N9)**: criado `api/lib/serginho/analysis/githubActionRecommendations.js` com `isActionRecommendationFollowUp(message)` (detecta perguntas de recomendação/priorização/próximos passos em PT-BR e EN via regex: "próximos passos", "qual a prioridade", "o que deveria ser melhorado primeiro", "give me recommendations", "what should I do next", etc.), `hasEnoughContextForRecommendations(githubContext)` (verifica se há ao menos um campo de contexto disponível — mesmo critério do N6), `buildRecommendationPrompt(message, githubContext)` (monta prompt estruturado com contexto atual + contexto anterior se disponível + instruções de priorização em Alta/Média/Baixa + anti-alucinação), `formatRecommendationResponse(rawText, options)` (pós-processa resposta LLM: adiciona cabeçalho `## Recomendações`, rodapé com fonte, truncamento seguro com `[resposta truncada]`, redação de tokens `sk-*`), `getInsufficientRecommendationContextMessage()` (mensagem amigável orientando o usuário); orchestrator minimamente modificado com bloco recommendation follow-up ANTES dos blocos N8 e N6, guarda `_skipRecommendationCheck` anti-loop, `_meta.recommendationFollowUp = true` e `_meta.recommendationFormatted = true`; 76 novos testes. Zero dependências novas. Zero breaking changes. Rollback com `git revert`. (este PR)
